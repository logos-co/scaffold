...
logos-scaffold test-node start [--project DIR] [--state DIR] [--port N] [--work-dir DIR] [--preserve-work-dir] [--block-create-timeout-ms MS] [--retry-pending-blocks-timeout-ms MS] [--json]
logos-scaffold test-node status --node <id|dir> [--json]
logos-scaffold test-node stop --node <id|dir> [--preserve-work-dir]
logos-scaffold test-node run [--project DIR] [--state DIR] [--block-create-timeout-ms MS] [--retry-pending-blocks-timeout-ms MS] [--serial | --parallel N] -- <command...>
...
- `basecamp build` builds the `.lgx` artefacts for every `role = "project"` entry in `[modules]` without installing them. `--variant lgx|lgx-portable|all` selects the flake output(s) to build (default `all`); `--module NAME` restricts the build to a single captured project module. Output order is topological by `metadata.json` dependencies, same as `build-portable`.
- `basecamp build-portable` rebuilds every `role = "project"` entry in `[modules]` with attr-swapped `#lgx-portable` for hand-loading into a basecamp AppImage (alias for `build --variant lgx-portable`). Sources come from scaffold.toml (managed via `basecamp modules`); `--module NAME` restricts the build to a single captured project module. `role = "dependency"` entries are intentionally skipped — the target AppImage provides its own release companion modules via its Package Manager catalog. Output is ordered topologically by `metadata.json` dependencies (leaves first, so basecamp's AppImage can resolve each module's deps before loading it), and symlinked into `.scaffold/basecamp/portable/` as `<NN>-<module_name>.lgx` so the AppImage's "install lgx" file picker has browsable, human-named files in the right order. The directory is wiped and recreated per run.
- `basecamp run <module>` runs a captured module's standalone app via `nix run`, resolved from `[modules.<module>]` — `nix run <flake>` (the flake's default app), or `#<standalone_app>` when configured. `--host standalone` is the only host today and the default when omitted.
...
- `basecamp paths <profile>` prints the profile's resolved path manifest — XDG config/data/cache dirs, runtime dir, module/plugin dirs, launch state, log file, and env file — without building or mutating anything. `--json` for machine parsing.
- `basecamp docs` prints the canonical project-compatibility rules, the embedded copy of [docs/basecamp-module-requirements.md](./docs/basecamp-module-requirements.md).
