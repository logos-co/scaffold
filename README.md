...
- `create` and `new` are aliases. `--lez-path <PATH>` seeds the scaffold from a local lez checkout, but the supplied path is cloned and then reset to scaffold's pinned lez SHA — commits or uncommitted work above that pin will not appear in the new project. When the supplied path's HEAD differs from the pin, scaffold prints a warning naming both SHAs. To dogfood your local lez work, override `[repos.lez].pin` in the new project's `scaffold.toml` after creation and re-run `lgs setup`.
