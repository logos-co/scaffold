...
- A second `launch alice` while the first is still running terminates the first, leaving no orphan behind. Check with `pgrep -fl LogosBasecamp` (or `ps -o comm=`) before and after: **basecamp 0.2.x's dev build reports the process as `.LogosBasecamp`**, because `bin/LogosBasecamp` is a wrapper that `exec`s the hidden real binary; the portable stacks report `LogosBasecamp`. Both are expected — a surviving process of either name after the second launch is the regression.
