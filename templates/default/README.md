...
- `logos-blockchain-circuits`: resolved automatically from the `[circuits]`
  table in `scaffold.toml` — `setup`/`build`/`localnet` download the pinned
  release into `.scaffold/circuits` on demand. Set
  `LOGOS_BLOCKCHAIN_CIRCUITS=<path>` only to override with a local checkout.
...
Run each phase yourself when debugging a single step:
