...
fn build_idl_inner(force: bool) -> DynResult<()> {
    ...
    if project.config.framework.kind == FRAMEWORK_KIND_SPEL {
        // spel owns IDL generation for its own projects: delegate to the
        // project-vendored binary rather than duplicating the pipeline here.
        // `force` is irrelevant — `spel generate-idl` reads the source every
        // time and has no cache of its own.
