...
| Project | `build [project-path]` | Runs `setup`, then builds. For `spel` projects that is `make build` (the Docker guest build) — `cargo build --workspace` panics there. Otherwise `cargo build --workspace`, auto-compiling `methods/Cargo.toml` if present. |
