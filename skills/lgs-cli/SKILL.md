...
| Project | `build [project-path] [--guest {local,docker}]` | Runs `setup` then `cargo build --workspace`; auto-compiles `methods/Cargo.toml` if present. Guest programs build with the host toolchain by default (fast, non-reproducible `program_id`); `--guest docker` / `[build].guest = "docker"` builds them in the pinned risc0 container for a reproducible `program_id`. |
