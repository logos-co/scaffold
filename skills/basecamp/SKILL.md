...
| `lgs basecamp doctor [--json]` | Basecamp-specific health: captured-modules summary, manifest variant check per profile, drift between `[modules]` and on-disk profile state. |
...
For each project source's declared `dependencies`, the resolution order is: already-keyed → no-op; basecamp **preinstalls** (`capability_module`, `package_manager`, `counter`, `webview_app`, and their `_ui` siblings) → silent skip; declaring source's own `flake.lock` → use the locked rev rewritten to `#lgx`; scaffold-default `BASECAMP_DEPENDENCIES` table → fallback; **unresolved** → fail fast naming the dep and the two user-side fixes (capture as a project source, or add an explicit `[modules.<name>]` with `role = "dependency"`). No silent drop.
...
- `lgs basecamp doctor` (+ `--json`) — captured-modules summary, manifest variant check per seeded profile, drift between `[modules]` and on-disk profile state.
