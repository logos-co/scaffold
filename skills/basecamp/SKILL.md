...
3. **At least one `flake.nix`** — at the project root or in immediate sub-directories — exposing `packages.<system>.lgx`. This is the convention from `logos-module-builder`, still emitted by its 0.3.x releases (implemented there on top of `nix-bundle-lgx`). Scaffold supports only basecamp 0.3.0, so build the module with `logos-module-builder` 0.3.0: the pinned `lgpm` validates Merkle content hashes on install (tutorial-era packages carry none), and basecamp 0.3.0 refuses calls from modules linked against an older `logos-protocol`.
4. **For portable builds** (`build --variant lgx-portable|all` or `build-portable`): the same flakes also expose `packages.<system>.lgx-portable`. There is no silent fallback from `#lgx-portable` to `#lgx` — variant choice is the user's; if `#lgx-portable` is missing, the command fails with a targeted hint.
...
| `lgs basecamp modules [--show] [--flake REF]… [--path PATH]…` | **Sole writer of `[modules.*]` in `scaffold.toml`.** Auto-discovers project sub-flakes that expose `#lgx`, or takes explicit `--flake` / `--path` sources. Resolves `metadata.json` `dependencies` recursively. Manual edits in `[modules]` are preserved across re-runs. `--show` prints the captured set without mutating state. |
| `lgs basecamp install [--print-output]` | Build every captured source (`role = "project"` and `role = "dependency"`) and shell out to `lgpm` to install into both profiles. Logs to `.scaffold/logs/<ts>-install.log`; `--print-output` (or `LOGOS_SCAFFOLD_PRINT_OUTPUT=1`) streams nix output instead. If `[modules]` is empty, transparently invokes `modules` in auto-discover mode first. |
| `lgs basecamp launch <profile>` | Profile is `alice` or `bob`. Kills any prior `logos_host` / `LogosBasecamp` descendants for that profile, scrubs the profile's XDG dirs under `.scaffold/basecamp/profiles/<profile>/`, replays each captured source's install, sets `LOGOS_PROFILE` + `XDG_{CONFIG,DATA,CACHE}_HOME` + `LOGOS_USER_DIR`, and `exec`s basecamp. Clean-slate on every launch — that's the point, and it also discards the profile's `module_data/` and basecamp's own `logs/`. |
| `lgs basecamp develop <module> [--dev-shell ATTR]` | Enter the captured module's Nix dev shell, using the default shell unless an attribute is supplied. |
| `lgs basecamp build [--variant lgx\|lgx-portable\|all] [--module NAME]` | Build captured project modules without installing them. Defaults to both variants and writes load-ordered symlinks under `.scaffold/basecamp/lgx/` and `.scaffold/basecamp/portable/`. |
| `lgs basecamp build-portable [--module NAME]` | Back-compatible alias for `build --variant lgx-portable`; builds project entries in dependency order and writes `.scaffold/basecamp/portable/<NN>-<module_name>.lgx`. |
| `lgs basecamp run <module> [--host standalone]` | Run a captured module's standalone flake app with `nix run`; `standalone` is the only host and the default. |
| `lgs basecamp doctor [--json]` | Basecamp-specific health: basecamp/`lgpm` pin set, captured-modules summary, manifest variant check per profile, drift between `[modules]` and on-disk profile state. |
| `lgs basecamp paths <profile> [--json]` | Print the resolved profile path manifest without invoking Nix or mutating state. |
...
- **`role = "project"`** — a module the developer is building locally. `build --variant lgx-portable|all` and its `build-portable` alias attr-swap these to `#lgx-portable`.
- **`role = "dependency"`** — a runtime companion. `install` / `launch` load them; `build` and `build-portable` skip them.
...
For each project source's declared `dependencies` and `optional_dependencies` (plain names or `{name, version, signer}` objects), the resolution order is: already-keyed → no-op; modules **basecamp bundles itself** (`capability_module`, `modules_state`, `package_downloader`, `package_manager`, `package_manager_ui`) → silent skip; declaring source's own `flake.lock` → use the locked rev rewritten to `#lgx`; scaffold-default `BASECAMP_DEPENDENCIES` table → fallback; **unresolved** → fail fast naming the dep and the two user-side fixes (capture as a project source, or add an explicit `[modules.<name>]` with `role = "dependency"`). No silent drop — except for an optional dependency, which is skipped with a note.
...
| `.scaffold/basecamp/portable/<NN>-<name>.lgx` | Symlinks to `.#lgx-portable` builds, ordered by dependency topology. Wiped by each portable build. |
...
1. Kill any prior `logos_host` / `LogosBasecamp` descendants for the alice profile.
...
**Empty `[modules]` + `launch`** intentionally **bails before scrubbing**. The empty-install + scrubbed-profile combo would leave the profile with zero modules — capture modules first via `lgs basecamp modules`.
...
## `build` / `build-portable` (B5)

`build` creates `#lgx`, `#lgx-portable`, or both variants without installing them; it defaults to both and can target one captured project module with `--module`. `build-portable` is the back-compatible alias for `build --variant lgx-portable`. Portable artefacts can be hand-loaded via the AppImage's "install lgx" file picker rather than installed into scaffold-managed profiles.

Behavior: builds **only** `role = "project"` entries (the dev's local modules), in topological dependency order so basecamp can resolve each module's deps before loading it. `role = "dependency"` entries are skipped — the target host provides its own copies. Symlinks land in `.scaffold/basecamp/lgx/` or `.scaffold/basecamp/portable/` as `<NN>-<module_name>.lgx` (`NN` is the load-order index). Each selected directory is wiped and recreated, so removing a module via `basecamp modules` doesn't leave stale symlinks.
...
| `basecamp install` fails inside `nix build` with `no 'main' field in metadata.json` | A sub-flake transitively pulls a second `logos-module-builder` (typically off `main`) that packages modules differently than the one the project pins. The stale entry silently wins through the sub-flake's `flake.lock`. | In the offending sub-flake's `flake.nix`: add `inputs.<dep>.inputs.logos-module-builder.follows = "logos-module-builder";` for each dep that itself declares `logos-module-builder`, then `nix flake update`. Verify only one `logos-module-builder` node remains in `flake.lock`. |
| `basecamp install` fails with `Missing content hashes in manifest` / `Package validation failed` | The `.lgx` was built by pre-0.2 module tooling; the pinned `lgpm` validates structure and content hashes on install. | Rebuild the module with `logos-module-builder` 0.3.0 (or bundle with `nix-bundle-lgx`), or bump that `[modules.<name>].flake` rev. Do **not** downgrade `[repos.lgpm].pin` — basecamp 0.3.0 embeds the same validating library. |
| `basecamp install` fails with `Forbidden root entry: assets` | The `.lgx` (module-builder 0.3.x ships package icons under `assets/`) is newer than the pinned `lgpm` — the project pins an older `[repos.lgpm]`. | Run `lgs basecamp doctor`; move `[repos.basecamp]` / `[repos.lgpm]` back to scaffold's default set (`lgs basecamp setup` does it for retired defaults). |
| `basecamp install` fails with `Package does not contain variant for platform` | Dev/portable mismatch: a dev basecamp loads `<host>-dev` variants, a portable one bare `<host>` variants. | Use the matching flake output (`#lgx` for dev, `#lgx-portable` for portable) or align `[repos.basecamp].attr`. |
| `basecamp modules` fails with an unresolved-dep error | A `metadata.json` `dependencies` entry isn't already keyed in `[modules]`, isn't a module basecamp bundles, isn't in the source's `flake.lock`, and isn't in the scaffold default table. | Either add the dep as a flake input in the source's `flake.nix` (preferred — gives an authoritative pin), or hand-add `[modules.<name>]` with `flake = "<ref>#lgx"` and `role = "dependency"`. |
| `basecamp install` succeeds but `doctor` flags drift | `[modules]` was edited or sources changed without a re-install. | `lgs basecamp install`. |
...
| `build-portable` fails with missing `#lgx-portable` attr | A captured `role = "project"` flake doesn't expose the portable variant. | Add the `lgx-portable` output to that flake, or remove the entry from `[modules]` if not actually a project source. |
...
- `lgs basecamp doctor` (+ `--json`) — basecamp/`lgpm` pin set, captured-modules summary, manifest variant check per seeded profile, drift between `[modules]` and on-disk profile state.
...
- **B2** — module capture, install, resolved paths, and single-instance launch (`alice`).
- **B3** — two-instance p2p (`alice` + `bob` parallel terminals).
- **B4** — clean-slate scrub semantics on relaunch; empty-`[modules]` bail guard.
- **B5** — `build` variant selection and `build-portable` AppImage artefacts.
- **B6** — captured-module standalone run loop.

When reproducing a basecamp failure, name the matching scenario.

## Key Rules

- **Never edit `[modules]` while `lgs basecamp modules` is running.** Outside that window, hand-edits are preserved across re-runs — manual entries always win over derived pins.
...
- **`[repos.basecamp].pin` and `[repos.lgpm].pin` move as a set.** Basecamp embeds the same package-manager library `lgpm` is built from, so scaffold's default lgpm rev is the one the default basecamp release locks. Bumping one alone lets the installer and the app disagree about package format — `basecamp doctor` warns on a split pair. Companion pins in `BASECAMP_DEPENDENCIES` are scoped to the same release.
- **Only explicit portable build selections touch `#lgx-portable`.** Those are `build --variant lgx-portable|all` and its `build-portable` alias; `install` / `launch` always use `#lgx`. Silent variant fallback in either direction is a contract violation.
