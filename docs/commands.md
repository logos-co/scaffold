...
- `localnet reset` stops the sequencer, clears sequencer chain state, restarts, and verifies blocks. The chain lives in the project, at `.scaffold/localnet/` (`rocksdb/` plus the sequencer's signing key), so two projects on the same LEZ pin never share one chain and a reset in one leaves the other alone. Destructive: `--yes` is required unless `--dry-run` is passed (`--dry-run` prints the plan without changing anything). `--reset-wallet` also deletes the project wallet home and default-address state (irrecoverable).
...
- `spel -- ...` forwards raw spel CLI arguments to the project-vendored `spel` binary so any spel subcommand (`inspect`, `pda`, `generate-idl`, …) runs against the project's pinned version without a global install. It exports the project wallet home (`NSSA_WALLET_HOME_DIR` / `LEE_WALLET_HOME_DIR`) like `wallet -- ...`, so spel's IDL-driven instruction commands (`spel -- --idl idl/<program>.json --program <guest.bin> -- <instruction> ...`) sign and submit with the project wallet.
- `basecamp setup` pins basecamp + `lgpm` (read from `[repos.basecamp]` / `[repos.lgpm]` — both `build = "nix-flake"`), builds both (logged to `.scaffold/logs/<timestamp>-setup-*.log`), and seeds per-profile XDG directories for `alice` and `bob` under `.scaffold/basecamp/profiles/`. The two pins move as a set: scaffold's default `lgpm` rev is the one the pinned basecamp release locks, because that same package-manager library is what the app uses to read the modules `lgpm` installed. Scaffold supports basecamp 0.3.0 only; the defaults are that release's set. A project still on a default set an earlier scaffold wrote (basecamp 0.2.3 or v0.1.1, or a captured default `delivery_module` flake) is moved forward by `setup`, which prints each rewritten value; the match is on the exact retired revs (a deliberate pin to one is moved too); any other pin is kept, and moving it means editing both and re-running `setup`. A commit pin that is already built is reused rather than re-evaluated, so a second `setup` does not pay the basecamp flake's multi-GB evaluation again. Runtime config (`port_base`, `port_stride`) is in `[basecamp]`.
- `basecamp modules` is the sole writer of the captured module set, which lives in top-level `[modules.<name>]` sections (each with `flake` and `role = "project" | "dependency"`). Modules aren't basecamp's property — they're the project's Logos modules, which basecamp happens to be one consumer of. Zero-arg runs auto-discovery: walks project flakes (root `.#lgx` first, else immediate sub-flakes), derives a `module_name` per source (from `metadata.json.name` for local paths; heuristic from the github repo slug for remote refs, with a one-line assumption note you can correct in `scaffold.toml`), then resolves each declared dep name (plain names and 0.3.0's `{name, version, signer}` objects, from `dependencies` and `optional_dependencies`) by: (1) already keyed in `[modules]`, (2) the list of modules basecamp bundles itself, (3) the source's own `flake.lock`, (4) scaffold-default pin. Unresolved required deps **fail fast** — no silent skip; an unresolved optional dep is skipped with a note. `--flake <ref>` / `--path <file>` capture explicit project sources; `--show` prints the current set without mutating. Re-runs are idempotent: existing `[modules]` entries are preserved so hand-edits survive. Project contract: see [docs/basecamp-module-requirements.md](./basecamp-module-requirements.md).
...
- `basecamp run <module>` runs a captured module standalone via `nix run <flake>`, without starting basecamp itself. Like `install`, `build` and `develop`, it passes `--override-input <name> path:<sibling>` for each `path:../<sibling>` input that names another captured project module, so multi-flake projects resolve their siblings from the working tree.
