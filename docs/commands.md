...
- **Every command that rewrites `scaffold.toml` edits it in place through `toml_edit`**, so comments, key ordering, and sections scaffold doesn't model survive — only the keys scaffold owns are reassigned. This matters because these comments are frequently load-bearing (why a `runtime_dir` override exists, why a pin is held back); losing them silently re-opens the bug they were written to document. If the file is unparseable, scaffold falls back to a from-scratch render rather than refusing to write.
