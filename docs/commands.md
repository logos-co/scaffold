...
```bash
logos-scaffold basecamp setup [--inspector | --no-inspector]
...
logos-scaffold basecamp build [--variant lgx|lgx-portable|all] [--module NAME] [--print-output]
logos-scaffold basecamp build-portable [--module NAME] [--print-output]
...
- `basecamp setup` pins basecamp + `lgpm` (read from `[repos.basecamp]` / `[repos.lgpm]` — both `build = "nix-flake"`), builds both (logged to `.scaffold/logs/<timestamp>-setup-*.log`), and seeds per-profile XDG directories for `alice` and `bob` under `.scaffold/basecamp/profiles/`. The two pins move as a set: scaffold's default `lgpm` rev is the one the pinned basecamp release locks, because that same package-manager library is what the app uses to read the modules `lgpm` installed. Existing projects keep whatever they pinned in `scaffold.toml` — bumping means editing both pins and re-running `setup`. Runtime config (`port_base`, `port_stride`) is in `[basecamp]`. Setup also records what it built as `key=value` lines in `.scaffold/state/basecamp.state` — `pin`, `basecamp_bin`, `lgpm_bin` — which is the supported way to find the binary from a script. Use `basecamp_bin` verbatim rather than reconstructing a path: which file under the nix output is the right entry point differs by basecamp generation and by stack — on a dev `#app` build it is a `/bin/sh` launcher that exports `QT_PLUGIN_PATH` / `QML2_IMPORT_PATH`, and picking the raw binary beside it yields an app that cannot find its Qt platform plugin or QML imports. `setup` resolves that for you. `lgs basecamp doctor` reads the same file and prints the path for humans. [basecamp-module-requirements.md](./basecamp-module-requirements.md#what-setup-leaves-behind--scaffoldstatebasecampstate) — which `basecamp docs` prints offline — carries the full contract for script consumers, including the read-only out-link layout and the fact that the path moves when the stack does.
  - `--inspector` selects the Basecamp build with the QML inspector compiled in, so a UI harness can connect and drive the app headlessly — this is the build a sitometres UI end-to-end suite needs, driving Basecamp through the inspector via `logos-qt-mcp`. Read the built binary's path from `basecamp_bin` in `.scaffold/state/basecamp.state` (above) rather than deriving it — that is what a harness should exec. It is a test-only artefact: upstream deliberately leaves the inspector off in every shipping output, and `setup` says so when it selects it. The flag sets this host's basecamp attr and persists it, so later runs stay on that build without repeating the flag; `--no-inspector` restores the default. Where it writes depends on the form the project already uses: on a project with a per-platform `[repos.basecamp.attr]` map it writes this host's key into that map (the map is what gets serialized, so writing the scalar would persist nothing), and on a project without one it writes the scalar. Passing neither flag leaves whatever the project already configured alone. `--no-inspector` declines to touch an attr scaffold never selected — a hand-set `bin-appimage` is left alone, and scaffold says so rather than appearing to do nothing — and warns if an explicit toggle leaves `[repos.lgpm].attr` on a variant it does not manage. What `--no-inspector` restores is the *default* stack, not whatever was configured before `--inspector`: scaffold does not remember a displaced hand-set attr, so if you had one, set it again — the command prints a note when it lands on something other than the default. A flag that declines to act declines for the *pair*: a `--no-inspector` that leaves `[repos.basecamp].attr` alone also leaves `[repos.lgpm].attr` alone and writes nothing, since acting on the half the user cannot see would be the same silent mismatch in the other direction. Both flags edit only the current host's entry in a per-platform map — `setup` builds for the machine it runs on, so a Linux developer's toggle must not discard the project's `aarch64-darwin` mapping. Passing both flags at once is an error rather than a last-wins guess, matching `run`'s `--reset` / `--no-reset`: the choice persists to `scaffold.toml`, so silently resolving a fat-fingered command line would rewrite the project's stack with no diagnostic. Because the choice also decides the `[repos.lgpm]` variant — the inspector build is the portable stack, which needs `cli-portable` — `setup` moves both attrs together and writes them before the build, so an interrupted build cannot leave the two on different stacks. A mismatch there is quiet rather than loud: Basecamp logs one `variant ... not supported on this platform` line, declines to load the modules, and opens to an empty UI.
...
- `basecamp build` builds the project's `.lgx` artefacts without installing them. `--variant` picks `lgx`, `lgx-portable`, or `all` (the default); `--module NAME` restricts the build to one captured project module. `build-portable` is the alias for `build --variant lgx-portable`. Both accept `--print-output` (or `LOGOS_SCAFFOLD_PRINT_OUTPUT=1`), which streams nix output to the terminal instead of logging it and printing a one-line status — the same switch `install` takes. Use it in CI, where a failing module build otherwise reports the derivation failure without the compiler output explaining why.
