...
- **Every command that rewrites `scaffold.toml` edits it in place through `toml_edit`**, so comments, key ordering, and sections scaffold doesn't model survive — only the keys scaffold owns are reassigned. This matters because these comments are frequently load-bearing (why a `runtime_dir` override exists, why a pin is held back); losing them silently re-opens the bug they were written to document. An unparseable `scaffold.toml` never reaches the writer: every command loads and parses the file first, so a syntax error is a hard error that leaves the file untouched. (The writer still carries a from-scratch fallback, which warns and takes a timestamped `scaffold.toml.bak-*` copy before overwriting, but it is an internal guard against the file changing between load and write — not a recovery path a user can invoke.)
