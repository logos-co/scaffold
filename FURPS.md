...
10. Guest programs can be built reproducibly. `[build].guest = "docker"` (or `lgs build --guest docker`) compiles `methods/` guests with `cargo risczero build` inside the pinned `risczero/risc0-guest-builder:<tag>` container, so the same source produces the same ELF — and therefore the same `program_id` — on any machine or CI runner. The default remains the host-toolchain `embed_methods()` path, which is faster and needs no container runtime but is not bit-reproducible; `lgs build` says so once per run and `lgs doctor` reports the active strategy. `lgs deploy` prefers the deterministic artefact, and each `lgs build` clears the other mode's output, so the last build is always what deploy ships.
